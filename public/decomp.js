      ...
      1: [
        function (_dereq_, module, exports) {
          ...
          function lineSegmentsIntersect(p1, p2, q1, q2) {
            ...
            var s =
              (dx * (q1[1] - p1[1]) + dy * (p1[0] - q1[0])) /
              (da * dy - db * dx);
            var t =
              (da * (p1[1] - q1[1]) + db * (q1[0] - p1[0])) /
              (db * dx - da * dy);
              ...
              if (polygonIsReflex(polygon, i)) {
                for (var j = 0; j < polygon.length; ++j) {
                  if (polygonCanSee(polygon, i, j)) {
                    tmp1 = polygonGetCutEdges(
                      polygonCopy(polygon, i, j, tmpPoly)
                    );
                    tmp2 = polygonGetCutEdges(
                      polygonCopy(polygon, j, i, tmpPoly)
                    );
          ...
          function polygonQuickDecomp(
            ...
            if (level > maxlevel) {
              console.warn(
                "quickDecomp: max level (" + maxlevel + ") reached."
              );
              return result;
            }
              ...
              if (polygonIsReflex(poly, i)) {
                ...
                for (var j = 0; j < polygon.length; ++j) {
                  if (
                    ...
                    if (
                      isRight(polygonAt(poly, i + 1), polygonAt(poly, i), p)
                    ) {
          ...
          function points_eq(a, b, precision) {
            return (
              scalar_eq(a[0], b[0], precision) &&
              scalar_eq(a[1], b[1], precision)
            );
          }
        },
        {},
